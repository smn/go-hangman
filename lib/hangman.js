...
function Hangman() {
    ...
    self.get_random_word = function () {
        ...
        p.add_callback(function (result) {
            return result.body.toLowerCase().trim();
        });
        p.add_callback(function (word) {
            var lp = im.log('Word: "' + word + '"');
    ...
    self.game_logic = function (im, game, content) {
        ...
        if(!content) {
            ...
            im.log('checking', game.word, 'for', content);
    ...
    self.add_creator('new_game', function (state_name, im) {
        ...
        // NOTE: I suspect this is necessary because this state
        // is called twice
        p.add_callback(function (game) {
            if(game && !self.won(game)) {
                return game.word;
            } else {
